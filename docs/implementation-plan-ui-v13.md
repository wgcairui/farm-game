...
- **坐标系转换（必读）**：`plot-layout.json` 的 `center_720` 是**图像坐标（y 向下、原点左上）**；Cocos UI 是**中心原点、y 向上**。转换：`cocosX = center_720[0]`，`cocosY = 1280 − center_720[1]`。例：plot 0 → (162.5, **445.5**)，plot 23 → (537.5, **304.5**)。搞反 y 是本项目历史最爱踩的坑。
- 地块 sprite 显示尺寸：`88×88` 设计 px（`PLOT_DISPLAY`）；作物 sprite `56` 设计 px 宽，锚在格中心上移 `10` px（`CROP_Y_LIFT`）。
- **地块连片契约（2026-09-13 修订）**：24 格排成 6×4 的**无缝整块**耕地；格距取自贴图可见几何而非 sprite 盒——`x=75`（最窄的翻土贴图可见宽 74.9）、`y=47`（顶面 42.6 + 正面 4.5，不含投影）。行距 47 让每排盖住后排的投影、同时露出自己的正面，呈"梯田式"整片（原来 70.7×90 的格心距会在行间露出草缝，观感是散开的格子）。坐标由 `scripts/make-plot-layout.mjs` 生成（改 `--cols/--rows/--pitch-x/--pitch-y` 重跑即可），整块居中于底图留白区 `lawn_region_720`。
- **命中区 = 格距（75×47），不再用 85×85**：Cocos 命中测试取最上层命中节点，85px 盒在 47 行距下会让前排偷走后排的点击。格距与格子一一对应 → 零重叠零空隙。代价：单格触摸目标 39×24.5 物理pt（375pt 屏），小于 44pt；44pt 规则继续适用于 UI 按钮（v24 §6），地块属密集网格按格映射。需要更大触摸格就把整块放大（加大 pitch 参数）。
- **格内浮动元素**：倒计时标签 `46×18 @ (0, +23)`（原为浮在格上方 +60，连片后会压到后排地块；成熟角标留在格右上角，与倒计时互斥显示）。
