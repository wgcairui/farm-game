...
export interface FarmUnlockRoute {
  ...
  response: ApiResponse<FarmUnlockResponse>;
}
export interface FarmPlantRoute {
  path: '/farm/plant';
  method: 'POST';
  headers: { authorization: `Bearer ${string}` };
  body: FarmPlantRequest;
  response: ApiResponse<FarmPlantResponse>;
}
export interface FarmWaterRoute {
  path: '/farm/water';
  method: 'POST';
  headers: { authorization: `Bearer ${string}` };
  body: FarmWaterRequest;
  response: ApiResponse<FarmWaterResponse>;
}
export interface FarmHarvestRoute {
  path: '/farm/harvest';
  method: 'POST';
  headers: { authorization: `Bearer ${string}` };
  body: FarmHarvestRequest;
  response: ApiResponse<FarmHarvestResponse>;
}
